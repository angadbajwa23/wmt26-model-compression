...
BitsAndBytes q4 organizer baseline for WMT26 Model Compression.

- Base model: `google/gemma-3-12b-it`
- Runtime: PyTorch + Transformers + BitsAndBytes
- Model artifact: BNB q4 model at `workdir/model`, or set `MODEL_DIR` when running.
...
`compress.sh` is optional documentation/reproducibility support. It loads `google/gemma-3-12b-it` with `load_in_4bit=True` and saves the prepared artifact under `workdir/model` by default.
