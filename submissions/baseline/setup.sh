...
Create a Python 3.12 uv environment and install baseline inference requirements.

The uncompressed model is expected at workdir/model, or set MODEL_DIR when running.
compress.sh is a no-op for this baseline because no compression is applied.
...
if [[ -f "$model_dir/config.json" ]]; then
    ...
    echo "No model found at $model_dir; provide the uncompressed model there or set MODEL_DIR at run time." >&2
fi